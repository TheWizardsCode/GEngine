...
def render_summary_table(summary: dict[str, Any]) -> str:
    ...
    # Core stats table
    stats_table = Table(
        title="World Status", show_header=True, header_style="bold cyan"
    )
    ...
    if isinstance(stability, (int, float)):
        stab_color = (
            "green" if stability >= 0.7 else "yellow" if stability >= 0.4 else "red"
        )
        stats_table.add_row(
            "Stability", f"[{stab_color}]{stability:.3f}[/{stab_color}]"
        )
...
def _render_environment_panel(console: Console, impact: dict[str, Any]) -> None:
    ...
    pressure_color = (
        "red" if pressure > 2.0 else "yellow" if pressure > 1.0 else "green"
    )
    lines.append(
        f"[bold]Scarcity Pressure:[/bold] "
        f"[{pressure_color}]{pressure:.2f}[/{pressure_color}]"
    )
    ...
    if isinstance(avg_pollution, (int, float)):
        poll_color = (
            "red"
            if avg_pollution > 0.7
            else "yellow"
            if avg_pollution > 0.4
            else "green"
        )
        lines.append(
            f"[bold]Avg Pollution:[/bold] "
            f"[{poll_color}]{avg_pollution:.3f}[/{poll_color}]"
        )
    ...
    if isinstance(extremes, dict) and extremes:
        ...
        if max_info and min_info:
            ...
            lines.append(
                ...
                f"max [red]{max_info.get('district')}[/red] {max_poll:.3f}, "
                f"min [green]{min_info.get('district')}[/green] {min_poll:.3f}"
            )
    ...
    if isinstance(biodiversity, dict) and biodiversity.get("value") is not None:
        ...
        if isinstance(value, (int, float)):
            ...
            lines.append(
                f"[bold]Biodiversity:[/bold] "
                f"[{bio_color}]{value:.3f}{delta_text}[/{bio_color}]"
            )

    panel = Panel(
        "\n".join(lines),
        title="[bold cyan]Environment Impact[/bold cyan]",
        border_style="cyan",
    )
    console.print(panel)
...
def _render_focus_panel(console: Console, focus: dict[str, Any]) -> None:
    ...
    panel = Panel(
        "\n".join(lines),
        title="[bold magenta]Focus State[/bold magenta]",
...
def _render_digest_panel(console: Console, digest: dict[str, Any]) -> None:
    ...
    panel = Panel(
        "\n".join(lines),
        title="[bold green]Focus Digest[/bold green]",
        border_style="green",
    )
    console.print(panel)
...
def _render_story_seeds_panel(console: Console, seeds: list[dict[str, Any]]) -> None:
    ...
    table = Table(
        title="Active Story Seeds", show_header=True, header_style="bold yellow"
    )
    ...
    for seed in seeds[:5]:
        ...
        table.add_row(seed_id, title, f"[{status_color}]{status}[/{status_color}]")
...
def _render_profiling_panel(console: Console, profiling: dict[str, Any]) -> None:
    ...
    if anomalies > 0:
        ...
        table.add_row(
            "[bold]Anomalies[/bold]", f"[{anom_color}]{anomalies}[/{anom_color}]"
        )

    console.print(table)


def render_director_table(
    feed: dict[str, Any],
    ...
    if feed:
        ...
        panel = Panel(
            "\n".join(lines),
            title="[bold cyan]Current Director Feed[/bold cyan]",
            border_style="cyan",
        )
        console.print(panel)
        console.print()
    ...
    if analysis:
        ...
        if events:
            table = Table(
                title="Director Events", show_header=True, header_style="bold yellow"
            )
    ...
    if history:
        table = Table(
            title="Focus History", show_header=True, header_style="bold magenta"
        )
        table.add_column("Tick", style="dim", width=8)
        ...
        for entry in history[-5:]:
            ...
            table.add_row(str(tick), center, str(suppressed), top_msg)
...
def render_map_overlay(summary: dict[str, Any]) -> str:
    ...
    console.print(
        Panel("[bold cyan]City Map Overview[/bold cyan]", border_style="cyan")
    )
    console.print()
    ...
    if isinstance(focus, dict) and focus.get("district_id"):
        ...
        if coords:
            ...
            console.print(
                f"[bold]Focus Center:[/bold] {focus.get('district_id')} "
                f"at ({x_coord:.1f}, {y_coord:.1f})"
            )
