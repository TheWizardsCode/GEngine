...
class GatewaySession:
    ...
    def execute_natural_language(self, text: str) -> CommandResult:
        ...
        if not intent:
            ...
            if fallback:
                ...
                result = CommandResult(
                    output="I couldn't understand that command. "
                    "Try: summary, map, next, run <n>",
                    should_exit=False,
                )
        ...
        try:
            ...
            LOGGER.info(
                "Mapped intent %s -> command: %s", type(intent).__name__, command
            )
    ...
    def _build_intent_context(self, summary: dict[str, Any]) -> dict[str, Any]:
        ...
        if "focus" in summary and isinstance(summary["focus"], dict):
            focus_center = summary["focus"].get("district_id") or summary["focus"].get(
                "focus_center"
            )
            if focus_center:
    ...
    def _log_focus(
        ...
        *,
        ...
        director_history = (
            summary.get("director_history") if isinstance(summary, dict) else None
        )
        focus_center = None
        ...
        director_history_len = (
            len(director_history or []) if isinstance(director_history, Iterable) else 0
        )
        tick = summary.get("tick") if isinstance(summary, dict) else None
        LOGGER.info(
            "gateway session=%s label=%s tick=%s focus=%s "
            "suppressed=%s history=%s director_history=%s",
            self.session_id,
            label,
            tick,
