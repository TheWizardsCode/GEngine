...
description: Skill to implement an existing Beads issue by id. Handles `/implement` and `implement` invocations and updates the specified bead rather than creating new beads.
---

# Implement Skill

To run this skill, the agent should be invoked with a command like:
...
- Strip a leading slash-command token if present (e.g., `/implement`).
...
- Fetch the bead JSON with `bd show <id> --json` and evaluate whether the bead is actionable per the project `implement` workflow (see `.opencode/command/implement.md`).
...
- This SKILL.md provides the procedural instructions the agent needs to perform `/implement` actions programmatically. The runtime that loads skills should ensure this skill is selected when user input begins with `implement` or `/implement` and the first token after the command looks like a beads id.
...
- "Error: missing bead id. Run `implement <bead-id>` or `/implement <bead-id>`.">
